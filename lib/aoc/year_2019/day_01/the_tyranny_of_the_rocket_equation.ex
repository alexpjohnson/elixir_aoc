defmodule Aoc.Year2019.Day01.TheTyrannyoftheRocketEquation do
  ...
  *What is the sum of the fuel requirements* for all of the modules on your
  ...
  def part_2(input) do
    input
    |> String.split("\n", trim: true)
    |> Enum.map(fn x -> fuel_for_mass(String.to_integer(x), 0) end)
    |> Enum.sum
  end

  def fuel_for_mass(n, sum) when n <= 6 do
    sum
  end

  def fuel_for_mass(n, sum) do
    val = Integer.floor_div(n, 3) - 2
    fuel_for_mass(val, sum + val)
  end
end